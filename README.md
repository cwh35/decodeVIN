...
1HGCM82633A004352 5YJ3E1EA6PF384836 1FTFW1ET9DFC10312 1C4RJFBG2FC625797 5FNRL6H79NB021411 1HD1KBM15FB620271 1XPWD40X1ED215307
...
**Windows (PowerShell):** `curl` is aliased to `Invoke-WebRequest` in PowerShell, which doesn't accept `-X`/`-H`/`-d` the way real curl does. Use the native cmdlets instead:
```powershell
...
```
(Or call the real curl binary explicitly with `curl.exe`, escaping inner quotes: `curl.exe -X POST ... -d '{\"vin\":\"1HGCM82633A004352\"}'`.)
